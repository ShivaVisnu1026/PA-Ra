...
// BEAR Breakout Alert - Flat EMA + Consolidation + Volume Filter (Monthly Timeframe)
// Signals (ret = 1) when:
// 1) Monthly 20 EMA is flattish for the past N bars
...
// 6) Past 5 months average volume > 300
// Optimized for monthly timeframe
...
input: ema_period(20, "EMA Period"),
       ...
       ema_flat_bars(3, "EMA flat lookback bars (monthly)"),
       ema_flat_pct(0.50, "EMA flatness threshold (%)"),
       consol_bars(8, "Consolidation lookback bars"),
       consol_range_pct(5.0, "Consolidation range threshold (%)"),
       breakout_bars(5, "Breakout lookback bars"),
       min_avg_volume(300, "Minimum 5-month average volume"),
       vol_lookback(5, "Volume lookback months");
...
// ===== Timeframe Check: Must be monthly chart =====
is_monthly = (BarFreq = "M");
...
begin
    ...
    begin
        // ===== Check 1: Monthly 20 EMA is flattish for past N bars =====
        ...
        // ===== Check 5: Past 5 months average volume > 300 =====
        vol_sum = 0;
