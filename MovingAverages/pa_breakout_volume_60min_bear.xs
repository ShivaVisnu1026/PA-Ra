...
// BEAR Breakout Alert - Flat EMA + Consolidation + Volume Filter (60 Min Timeframe)
// Signals (ret = 1) when:
// 1) 60-min 20 EMA is flattish for the past N bars
...
// Optimized for 60-minute timeframe
...
input: ema_period(20, "EMA Period"),
       ...
       ema_flat_bars(8, "EMA flat lookback bars (60-min)"),
       ema_flat_pct(0.30, "EMA flatness threshold (%)"),
       consol_bars(8, "Consolidation lookback bars"),
       consol_range_pct(1.5, "Consolidation range threshold (%)"),
...
// ===== Timeframe Check: Must be 60-minute chart =====
is_60min = (BarInterval = 60);

if not is_60min then
begin
    ret = 0;
end
else
begin
    ...
    begin
        // ===== Check 1: 60-min 20 EMA is flattish for past N bars =====
