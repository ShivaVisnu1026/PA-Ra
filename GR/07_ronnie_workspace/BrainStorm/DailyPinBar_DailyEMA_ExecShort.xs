{@type:autotrade}
// Daily Bearish Pin Bar + Daily EMA Down - Execution Script (Short)
// Entry: one tick below the reversal bar's low
// Stop:  one tick above the reversal bar's high
...
// ===== Daily EMA Slope Down =====
var: d_ema(0), d_ema_prev(0), isDailyDown(false);
...
// When reversal conditions occur on this bar, arm a short setup
if isBearishPin and isDailyDown and vol_ok then
begin
    signal_date  = date;
    signal_high  = high;
    signal_low   = low;
    entry_price  = AddSpread(signal_low, -1);  // one tick below low
    stop_price   = AddSpread(signal_high, 1);  // one tick above high
    signal_armed = true;
end;
...
begin
    // Enter short when price trades at or below the entry price
    if low <= entry_price then
    begin
...
if Position = -1 then
begin
    // Exit if price trades at/above stop price
    if high >= stop_price then
    begin
