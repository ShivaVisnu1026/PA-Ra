// ============================================================
// Cross-Period Bearish Reversal Screener
// ============================================================
// 描述：
//   基於 Nail Fuller 的 price action 定義，偵測最新 1、2、3 期 K 棒
//   合併後是否呈現 bearish reversal bar。符合任一條件即觸發。
//
// 條件：
//   1. 最新 1 期 K 棒是 bearish reversal（且必須是黑K：close < open）
//   2. 最新 2 期 K 棒合併起來是 Bearish reversal（且合併後必須是黑K）
//   3. 最新 3 期 K 棒合併起來是 Bearish reversal（且合併後必須是黑K）
...
var: singleRange(0),
     ...
     singleHighestInRange(0),
     isSingleReversal(false),
     isSingleBlackK(false);
...
var: combo2Open(0),
     ...
     combo2HighestInRange(0),
     isCombo2Reversal(false),
     isCombo2BlackK(false);
...
var: combo3Open(0),
     ...
     combo3HighestInRange(0),
     isCombo3Reversal(false),
     isCombo3BlackK(false);
...
// ============================================================
// 條件 1：最新 1 期為 Bearish Reversal Bar（必須是黑K）
// ============================================================
...
isSingleBlackK   = (close < open);  // 必須是黑K
...
begin
    ...
    singleHighestInRange = highest(high, tail_prominence_bars);

    isSingleReversal =
        isSingleBlackK  // 必須是黑K
        and (singleUpperShadowPct >= tail_min_pct)  // 上影線至少佔比
        and (singleLowerShadowPct <= opp_tail_max_pct)  // 下影線不超過佔比
        and (singleBodyPct <= body_max_pct)  // 實體不超過佔比
        and (singleBodyPosition <= body_pos_pct)  // 實體在下方（底部位置 <= 45%）
        and (high >= singleHighestInRange);  // 高點是近期最高
end
else
    isSingleReversal = false;

// ============================================================
// 條件 2：最新 2 期合併為 Bearish Reversal Bar（必須是黑K）
// ============================================================
...
isCombo2BlackK = (combo2Close < combo2Open);  // 合併後必須是黑K
...
begin
    ...
    combo2HighestInRange = highest(high, tail_prominence_bars + 1);

    isCombo2Reversal =
        isCombo2BlackK  // 合併後必須是黑K
        and (combo2UpperShadowPct >= tail_min_pct)
        and (combo2LowerShadowPct <= opp_tail_max_pct)
        and (combo2BodyPct <= body_max_pct)
        and (combo2BodyPosition <= body_pos_pct)
        and (combo2High >= combo2HighestInRange);
end
else
    isCombo2Reversal = false;

// ============================================================
// 條件 3：最新 3 期合併為 Bearish Reversal Bar（必須是黑K）
// ============================================================
...
isCombo3BlackK = (combo3Close < combo3Open);  // 合併後必須是黑K
...
begin
    ...
    combo3HighestInRange = highest(high, tail_prominence_bars + 2);

    isCombo3Reversal =
        isCombo3BlackK  // 合併後必須是黑K
        and (combo3UpperShadowPct >= tail_min_pct)
        and (combo3LowerShadowPct <= opp_tail_max_pct)
        and (combo3BodyPct <= body_max_pct)
        and (combo3BodyPosition <= body_pos_pct)
        and (combo3High >= combo3HighestInRange);
end
else
    isCombo3Reversal = false;
