...
// 3) Current low breaks below min low of prior 5 bars
// 4) Bearish close (Close < Open)
// 5) Past 5 days average volume > 300
// 6) Upper wick > 30% of body
// 7) Price breaks below 20 EMA (using EMA flatness as breakout level)
...
begin
    ...
    // ===== Check 5: Upper wick > 30% of body =====
    body_size = Open - Close;
    if body_size > 0 then
    begin
        upper_wick = High - Open;
        wick_pct = (upper_wick / body_size) * 100;
    ...
    // ===== Check 6: Price breaks below 20 EMA (using EMA flatness as breakout level) =====
    breaks_below_ema = (Close < ema20_val);
    ...
    // ===== Final Signal =====
    if is_ema_flat and is_consolidated and is_breakout and is_bearish and wick_ok and breaks_below_ema and vol_ok then
    begin
